...
### 코드 리뷰 수정 (P0 3건 + P1 5건)
- **P0**: asyncio.Lock lazy init, `date.today()` → ET 기준 (4개 파일), Forward P/E 체인 비교 오류
- **P1**: sync 전략 메모리 캐시 복원, HealthMonitor API 중복 제거, 센티멘트 캐시 정리, wl_cooldown 만료 정리, 시장가 폴백 정규장 가드

### 엔진 복기 결과
- **거래 0건**: KIS 실거래 전환(2/28) 후 주말이라 정규장 미경험
- **KIS API HTTP 500**: 잔고 조회 6,805건 실패 — 주말 API 제한 추정, 월요일 장 개장 시 확인 필요
- **장외 API 호출 억제**: `_portfolio_sync_loop` CLOSED 세션 시 30초 → 5분 대기 (호출 96% 감소)
...
## [2026-02-25] WS/REST 보유종목 전용 정책

### commit `79c14b5`
- screener 루프: 스크리닝 결과 WS 구독 제거 (복기용 로그만 유지)
- _run_rest_price_feed(): screened_symbols 제거 → 보유종목 WS 백업만
- 테마 탐지 / 뉴스 임팩트: WS 구독 제거
- WS 구독 수: 최대 40종목 롤링 → 보유종목 2~7개만
- REST 호출: WS 정상 시 0회, 비정상 시 보유종목만 폴링 (45초)
- 근거: 장중 신규 매수 신호 없음 → 스크리닝 종목 실시간 데이터 불필요
...
### 문제
프리장(08:20) 아침 스캔 시 KIS 수급 API가 0종목 반환 → LCI 점수 0 → SEPA 전 종목 탈락

### 해결
1. **pykrx 전일 수급 폴백** (`swing_screener.py`)
   - KIS 0종목 시 `get_market_net_purchases_of_equities()` 자동 호출
   - KOSPI + KOSDAQ, 외국인 + 기관합계 4번 호출 → 0.8초, 2661종목
   - 전날 날짜 자동 계산 (주말 → 금요일 조정)

2. **SEPA min_score 60 복원** (`default.yml`)
   - 원설계 "min_score 60 기준 충분한 변별력" 복원 (70으로 올라가 있던 것 수정)

3. **LCI 미계산 시 중립값** (`sepa_trend.py`)
   - 수급 데이터 완전 미존재 시 0pt → 5pt (데이터 없음 ≠ 수급 없음)

### 결과
아침 스캔 시그널: 0개 → 2개 (효성티앤씨 RSI2 78pt, 대우건설 SEPA 65pt)

**커밋**: `ae1dd88`

---

## [2026-02-25] 미국증시 레포트 날짜 표기 개선

### 변경
- 헤더 날짜: `2026.02.25 07:00 기준 (전일 NY 마감)` → `2026.02.24 NY 마감 (KST 02/25 07:00 수신)`
- KST 전날 기준으로 US 마감일(현지) 계산, 주말 자동 조정 (토→금, 일→금)
- 차트 이미지 / S&P500 맵 제목에도 동일 기준 적용
