...
class RiskManager:
    ...
    def __init__(self, config: RiskConfig, initial_capital: Decimal):
        ...
        logger.info(
            ...
            f"최대포지션={config.max_positions}개 (동적={'ON' if config.dynamic_max_positions else 'OFF'}, "
            f"flex={config.flex_extra_positions}), "
    ...
    def can_open_position(
        ...
        # 3. (일일 거래 횟수 제한 제거 — 가용 현금이 게이트)

        # 4. (포지션 수 제한 제거 — 가용 현금이 게이트 역할)
