...
- **종목 마스터 DB**: 3,708개 KRX 전체 종목 관리 (FinanceDataReader + pykrx)
- **뉴스 수집** (다중 소스):
  - 네이버 금융: 주요 뉴스
  - Daum 금융: 증권 뉴스 + 200~300자 요약
  - 매일경제 RSS: 증권 섹션 + 100~150자 요약
- **Yahoo Finance**: US 시장 오버나이트 데이터 (지수, ETF, 개별주)

### 시그널 시스템
- **뉴스 테마 탐지**: LLM 기반 뉴스 분석 → 테마 + 종목 임팩트 추출
  - 다중 소스 수집: Naver + Daum + 매경 RSS (88건 → SHA1 중복 제거 → 78건)
  - 종목 마스터 DB 연동: 3,708개 종목 자동 검증
  - 양방향 임팩트 스코어: -10~+10 (bullish/bearish)
- **업종지수 보정**: KIS 업종지수 등락률 → 테마 점수 동적 보정
- **US 오버나이트 시그널**: US 시장 마감 데이터 → 한국 테마 선행 부스트
- **뉴스 임팩트 스코어링**: 종목별 뉴스 센티멘트 점수화 (threshold=5)
