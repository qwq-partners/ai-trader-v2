...
- **가격 검증**: REST API 실시간 시세 재조회
  - 등락률 1~15% (13:30 이후 상한 8% — 오후 과열 방지)
  - 현재가 > 시가, 거래량 > 0
- **제한**: 쿨다운 10분/종목, 1회 최대 5개 시그널, 최대 8개 검증
...
### 청산 관리 (ExitManager)
- **1차 익절**: +2% → 30% 매도
- **트레일링**: +2% 도달 시 활성화, 고점 대비 -2.5% 이탈 시 청산
- **ATR 동적 손절**: 범위 3~6% (ATR 기반 자동 조정)
- **손절**: 4% (default) / 전략별 override 가능
